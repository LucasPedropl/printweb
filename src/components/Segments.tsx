...
export default function Segments() {
	...
	useEffect(() => {
		const interval = setInterval(() => {
			...
			setSelectedSegment((current) => {
				const currentIndex = segmentsContent.items.findIndex(
					(s) => s.name === current,
				);
				const nextIndex =
					(currentIndex + 1) % segmentsContent.items.length;
	...
	const activeSegment = segmentsContent.items.find(
					...
					<div className="order-1 lg:col-start-1 lg:row-start-1 flex flex-col justify-center">
						...
						<div className="mb-2 lg:mb-10">
							<label className="block text-sm font-semibold text-gray-700 mb-3">
								Escolha seu segmento para ver as
								funcionalidades:
							</label>
					...
					<div className="relative order-2 lg:col-start-2 lg:row-start-1 lg:row-span-2">
						<div className="aspect-square rounded-[40px] overflow-hidden shadow-2xl relative z-10">
							<motion.img
								...
								src={
									activeSegment?.image ||
									`https://picsum.photos/seed/${selectedSegment}/800/800`
								}
								alt={selectedSegment}
